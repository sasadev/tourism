...
class Admin::BaseController < ApplicationController
	...
	def authenticate!
		if  @current_admin = AdminUser.alive_record(id: session[:admin_user_id], deleted: 0)
			# form = Form.find_controllers(controller_name)
			# if form.present?
			# 	@form_read  = @current_admin.role_read?(form)
			# 	@form_write = @current_admin.role_write?(form)
			# 	unless @form_read
			# 		redirect_to admin_no_access_right_path
			# 	end
			# end
		else
	...
	# def force_https
	# 	unless request.ssl? or Rails.env.development?#s  or Rails.env.production?
